...
# Скільки секунд довіряти щойно надісланій команді, поки станція її не підтвердила.
#
# Станція застосовує команду НЕ миттєво: наступні 1-2 відповіді /main ще містять
# старе значення. Рідний веб-інтерфейс через це просто ігнорує дві наступні
# відповіді (ignoreCount = 2 у його postPageEvent). Без цього перемикачі в HA
# "вискакують назад": натиснув -> за секунду опитування повернуло старе значення.
PENDING_WRITE_TTL = 5.0

# Межа "обмеженого режиму" (16 А) — саме таке число зашите у веб-інтерфейс станції.
RESTRICTED_MODE_LIMIT = 16

# Струм, який виставляє веб-інтерфейс станції при УВІМКНЕННІ обмеженого режиму.
# Так, 12, а не 16 — це не помилка, а поведінка рідного інтерфейсу.
RESTRICTED_MODE_CURRENT = 12

# Струм, який виставляє веб-інтерфейс при ВИМКНЕННІ обмеженого режиму.
# Стеля при цьому піднімається до curDesign, але саме значення ставиться в 16 —
# струм не подвоюється сам собою, це робить користувач повзунком.
UNRESTRICTED_MODE_CURRENT = 16

# Запасна межа струму, якщо станція не повідомила curDesign.
FALLBACK_DESIGN_CURRENT = 32

# systemTime у станції — це годинник, який тікає щосекунди. Пишемо його стан
# не частіше, ніж раз на стільки секунд (див. sensor.py).
SYSTEM_TIME_MIN_INTERVAL = 60.0
